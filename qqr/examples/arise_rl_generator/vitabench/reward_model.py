"""
VitaBench Curriculum Agent Reward Model — RG-KL
...
- R_fmt  (0/1 gate): instructions is non-empty and rubrics is a non-empty list
- R_diff (0-2): triangular difficulty reward 2·max(0, 1 − |c − K/2| / (K/2)),
  c = Σ_i 𝟙(r_i ≥ γ)，r_i = α·s_i + (1−α)·𝟙[s_i=1]（K=8, γ=0.9, α=0.8）
- Training reward = R_tool · R_fmt · (1 + R_diff) ∈ [0, 3]
...
async def group_reward(args: Namespace, group: list[list[Sample]], **kwargs):
    ...
    1. Read the reward already computed during rollout
    2. Separate students from teachers
    3. Compute Δ_r = avg(teacher) - avg(student)
    4. Neutralise the teacher reward and set loss_mask=0
    5. Compute λ(Δ_r)
    6. Build guided_tokens for the student
    7. Write rg_kl_coef
    """
    if len(group) <= 1:
    ...
    # -- 1) Collect each sample's reward (already computed during rollout) --
    raw_rewards = []
    ...
    # -- 2) Compute Δ_r --
    student_rewards = [
        r for r, s in zip(raw_rewards, real_samples)
    ...
    # -- 3) Neutralise the teacher reward so GRPO normalisation is unaffected --
    for idx, sample_group in enumerate(group):
    ...
    # locate this group's coach memory
    group_memory = ""
    ...
    # build guided_tokens for every sample in the group
