# ARISE-RL Generator — DeepResearch（深度研究出题者）
...
单工具（`web_search`）深度研究任务的出题者训练。出题者先调用真实
web_search 探索某个主题，再生成 `query + rubrics`，每条 rubric 都必须
锚定在真实搜索观测上（tool-grounded rubric construction）。
