"""
Travel Curriculum Agent Reward Model — RG-KL

The generator reward was already computed in execute_and_evaluate in rollout.py (paper Eq. (1)-(5)):
- R_tool (0/1 gate): the trajectory contains real tool observations and expected_tools passes argument-level checks
- R_fmt  (0/1 gate): query, expected_tools and rubrics are all well formed
...
group_reward (RG-KL)：
- The k_s students take part in GRPO; the k_m teachers only supply Δ_r, with reward neutralised and loss_mask=0
- λ(Δ_r) = λ_0 · gate · warmup · cosine_decay
- Builds guided_tokens = [p_m_prompt, τ_s_response] for the student
- Writes sample.rg_kl_coef
"""
...
async def group_reward(args: Namespace, group: list[list[Sample]], **kwargs):
    ...
    for idx, sample_group in enumerate(group):
        ...
        logger.info(
            ...
            f"validity={meta.get('tool_validity_reward', '?')} "
